...
RESERVE_OPTION_MAP = {
    ...
    "standing-first": ReserveOption.GENERAL_ONLY,
    "standing-only": ReserveOption.GENERAL_ONLY,
}
STANDING_OPTION_NAMES = {"standing-first", "standing-only"}
TRAIN_ID_PREFIX = "ktx:v1:"
TRAIN_TYPE_MAP = {
    "ktx": TrainType.KTX,
    "all": TrainType.ALL,
    "saemaeul": getattr(TrainType, "SAEMAEUL", TrainType.ALL),
    "itx-saemaeul": getattr(TrainType, "ITX_SAEMAEUL", getattr(TrainType, "SAEMAEUL", TrainType.ALL)),
    "mugunghwa": getattr(TrainType, "MUGUNGHWA", TrainType.ALL),
    "nuriro": getattr(TrainType, "NURIRO", getattr(TrainType, "MUGUNGHWA", TrainType.ALL)),
    "itx-cheongchun": getattr(TrainType, "ITX_CHEONGCHUN", TrainType.ALL),
}
...
class PatchedKorail(Korail):
    ...
    def reserve(self, train, passengers=None, option=ReserveOption.GENERAL_FIRST, try_waiting=False, allow_standing=False):
        reserving_seat = True
        standing_only = False
        try:
            if not train.has_seat():
                if allow_standing and train_allows_standing(train):
                    seat_type = "1"
                    standing_only = True
                else:
                    raise SoldOutError()
                ...
                if train.has_general_seat():
                    seat_type = "1"
                elif allow_standing and train_allows_standing(train):
                    seat_type = "1"
                    standing_only = True
                else:
                    raise SoldOutError()
        ...
        payload = {
            ...
            "txtSeatAttCd4": "015",
            "txtSeatAttCd5": "000",
            "hidFreeFlg": "Y" if standing_only else "N",
            "txtStndFlg": "Y" if standing_only else "N",
            "txtMenuId": "11",
            "txtSrcarCnt": "0",
            "txtJrnyCnt": "1",
            "txtJrnySqno1": "001",
...
def train_allows_standing(train) -> bool:
    candidates = [
        getattr(train, "reserve_possible_name", None),
        getattr(train, "reserve_possible", None),
        str(train),
    ]
...
def normalize_train(train, index: int) -> dict[str, object]:
    return {
        ...
        "allows_standing": train_allows_standing(train),
...
def command_reserve(args: argparse.Namespace) -> None:
    ...
    allow_standing = args.seat_option in STANDING_OPTION_NAMES
    include_no_seats = args.include_no_seats or allow_standing
    trains = client.search_train(
        args.dep,
        args.arr,
        args.date,
...
def add_common_trip_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default="ktx",
        help="검색/예약할 열차 종류 (ktx, all, mugunghwa, itx-saemaeul, saemaeul, nuriro, itx-cheongchun)",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    reserve_parser.add_argument(
        "--seat-option",
        choices=sorted(RESERVE_OPTION_MAP),
        default="general-first",
        help="좌석 우선순위 또는 입석 우선/전용 예약 모드",
    )
